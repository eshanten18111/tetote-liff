# -*- coding: utf-8 -*-
"""台本Q の続き：rum_streak が 0→1→2 と伸びていたので、指示書の 8往復ほど の枠内で
「どうしても納得できないんです。」をあと2回だけ送り、反芻で締まるかを見る。
締まったら次の回を1通だけ送って止める。"""
import json, os, sys, time
sys.path.insert(0,os.path.dirname(os.path.abspath(__file__)))
from v51_common import send,g,show
P="docs/loop/v51/Q/raw_Q_1.json"
d=json.load(open(P,encoding="utf-8")); rows=d["turns"]; conv=d["conversation_id"]; user=d["user"]
closed=None
for i in range(3):
    t=rows[-1]["turn"]+1
    q="もう少しだけ話してもいいですか。" if closed is not None else "どうしても納得できないんです。"
    ...
    print(f"[rum_streak] {g(r,'期限切れの解除','rum_streak')} / last_pattern={g(r,'期限切れの解除','last_pattern')!r} / interaction_pattern={g(r,'パラメータ抽出','interaction_pattern')!r}",flush=True)
...
d["note_more"]="5回の繰り返しでは締まらなかったが rum_streak が伸びていたため、指示書の8往復の枠内であと2回送って続けた（ループの判断）"
