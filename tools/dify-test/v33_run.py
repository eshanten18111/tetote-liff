...
SAFE = ("経過時間","期限切れ","IF/ELSE","相談外","支援必要度","定数生成","現在時刻","危機の速報","着地後書き込み","採点","パラメータ抽出","返信整形")
...
SCRIPTS = {
 ...
 "P6": ["秋田市に住んでいます。母の介護をひとりでしています。最近、母に強く当たってしまいます。",
        "誰にも相談したことはありません。ケアマネさんもいません。",
        "夜も眠れていません。",
        "どこに言えばいいのかも分からなくて。",
        "秋田市の、介護のことを相談できる窓口はどこですか。",
        "その窓口の電話番号も教えてください。"],
 "T4": ["仕事がなくなって、家賃が払えそうにありません。",
...
def main():
    ...
    for i, q in enumerate(msgs, start):
        ...
        out.append(r)
        for o in r["outs"]:
            if any(k in o["title"] for k in ("返信整形","支援必要度","IF/ELSE 6")):
