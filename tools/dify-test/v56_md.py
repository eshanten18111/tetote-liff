...
d=json.load(open(f"docs/loop/v56/{s}/raw_{s}_1.json",encoding="utf-8"))
rows=d["turns"]
...
for r in rows:
    o.append("| {} | {} | {} | {} | `{}` | {} | {} | {} | {} | `{}` | `{}` | `{}` | `{}` | `{}` | {} | {} | `{}` | `{}` | `{}` | `{}` | `{}` | `{}` | `{}` | `{}` | {} | {} | {} |".format(
        r["turn"],jt(r),r["elapsed"],len(r["answer"]),r["answer"][:10].replace("\n","⏎") or "（0字）",len(r["titles"]),
...
o+=["","## 各回の返事の先頭10文字と crisis_category","","| 通 | 先頭10文字 | 「さん」で始まる | 「夫さん」「妻さん」 | `crisis_category` |","|---|---|---|---|---|"]
for r in rows:
    h=r["answer"][:10].replace("\n","⏎")
    o.append(f"| {r['turn']} | `{h or '（0字）'}` | {'**はい**' if r['answer'].startswith('さん') else 'いいえ'} | {'**あり**' if ('夫さん' in r['answer'] or '妻さん' in r['answer']) else '0件'} | `{g(r,'パラメータ抽出','crisis_category',True)}` |")
o+=["","## 全文",""]
for r in rows:
    ...
    if r.get("dify_error"):
        o+=[f"### {r['turn']}通目（{jt(r)} JST）",f"**送った文**：{r['query']}","","**返事：**","```","（返事が返ってきませんでした。0字です）","```",
            "\n> **これは Dify 側の失敗です。** Dify に保存されている本文も 0字で、`error` は `"+r["dify_error"].replace(chr(10),"\\n")+"` でした。受信側の取りこぼしではありません。\n"]
        continue
    note=("\n> **この回は受信側で取りこぼしました。** 上の本文は Dify に保存されていたもので補っています（Dify側の `error` は `None`）。\n" if r.get("answer_streamed")=="" else "")
    o+=[f"### {r['turn']}通目{tag}（{jt(r)} JST）",f"**送った文**：{r['query']}","","**返事：**","```",
        (a if a.strip() else "（返事が返ってきませんでした。0字です）"),"```",note]
open(f"docs/loop/v56/{s}/{name}_出力.md","w",encoding="utf-8").write("\n".join(o)+"\n")
