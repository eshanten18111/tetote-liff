...
user="t-35-L"; conv=None; rows=[]; fi=0; prev=""
for turn in range(1,27):
    ...
    if "次にお話しできるのは" in r["answer"] and turn<25:
        ...
        if turn<25: break
    time.sleep(1)
os.makedirs("docs/loop/v35/L",exist_ok=True)
json.dump({"script":"L","user":user,"conversation_id":conv,"turns":rows},
          open("docs/loop/v35/L/raw_L_1.json","w",encoding="utf-8"),ensure_ascii=False,indent=1)
print("会話ID:",conv)
