...
user="t-36-L"; conv=None; rows=[]; fi=0; prev=""
for turn in range(1,27):
    ...
    print(f"--- {turn}通目 送={q} / {r['elapsed']}秒 / {len(r['answer'])}字 / node{len(r['titles'])} / length_alerted={la!r} / forced_close={fc!r} / IF6={i6v} / need={(pa[0].get('support_need') if pa else '-')}",flush=True)
...
json.dump({"script":"L","user":user,"conversation_id":conv,"turns":rows},
          open("docs/loop/v36/L/raw_L_1.json","w",encoding="utf-8"),ensure_ascii=False,indent=1)
print("会話ID:",conv)
