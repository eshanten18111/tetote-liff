...
d=json.load(open("docs/loop/v42/C/raw_C_1.json",encoding="utf-8"))
rows=d["turns"]
...
out.append(f"- 行の先頭に来た記号・数字（箇条書きではない行も含む）： {bullets}")
...
for r in rows:
    for u in re.findall(r"https?://\S+", r["answer"]):
        ...
        out.append(f"- {r['turn']}通目：長さ{len(u)}字／形={'ふつう' if clean else '★あやしい'}／先頭20字（ドメインだけ） `{u[:20]}…`（アドレスの残りは書き写していません）")
...
for r in rows:
    ...
    out+= [f"### {r['turn']}通目"+("（同意の回）" if r["turn"]==d["consent_turn"] else ""),
           f"**送った文**：{r['query']}","","**返事：**","```",a,"```",""]
open("docs/loop/v42/C/台本C_出力.md","w",encoding="utf-8").write("\n".join(out)+"\n")
print("\n".join(out[:80]))
