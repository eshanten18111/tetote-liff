...
seen=set(); G={}
for f in sorted(glob.glob('docs/loop/v35/*/raw_*.json')):
    d=json.load(open(f)); who=d['user']
    if who=='t-35-MSG': who='t-35-'+f.split('raw_')[1].split('_')[0]
    for t in d['turns']:
        k=(who,int(t['turn']))
        if k in seen: continue
        seen.add(k); G.setdefault(who,{})[int(t['turn'])]=(t,d['conversation_id'])
for who,turns in G.items():
    name=who.replace('t-35-','')
    os.makedirs(f'docs/loop/v35/{name}',exist_ok=True)
    out=f'docs/loop/v35/{name}/台本{name}_出力.md'
    conv=list(turns.values())[0][1]
    L=[f"# 台本{name} 生出力（第34版）","",f"- 利用者ID: `{who}`",f"- 会話ID: `{conv}`",
       "- 版の目印: `定数生成（true文字列）／版 2026-09-20a`","","本文は一字も直していません。","","---",""]
