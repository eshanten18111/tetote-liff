# -*- coding: utf-8 -*-
"""第48版 台本R：危ないことばが、鍵を越えて届くこと（最優先）。
...
user="t-48-R"; conv=None; rows=[]; prev=""; si=0; ai=0; stop=""; consent=None
...
for turn in range(1,21):
    ...
    if consent is not None and ai==2:   # ＋2通目＝危ないことば
        teikei = (len(r["answer"])==301 and not any(t=="LLM" for t in r["titles"]))
        print(f"[R-2 判定用] LLM を通った={'はい' if any(t=='LLM' for t in r['titles']) else '★いいえ'} / crisis_word={g(r,'期限切れの解除','crisis_word')!r} / rescore={g(r,'期限切れの解除','rescore')!r} / 字数={len(r['answer'])} / node数={len(r['titles'])}",flush=True)
        print("[＋2通目 通ったノード]",flush=True)
        for t in r["titles"]: print("   ",t,flush=True)
        if teikei:
            stop="★ ＋2通目が決まった短い返事のままでした（R-2 ×）。指示書のとおりここで止めます"
            print(stop,flush=True); break
    time.sleep(1)
print("止めた理由:",stop or "流しきりました")
print("同意の回:",consent)
os.makedirs("docs/loop/v48/R",exist_ok=True)
json.dump({"script":"R","user":user,"conversation_id":conv,"consent_turn":consent,"turns":rows},
          open("docs/loop/v48/R/raw_R_1.json","w",encoding="utf-8"),ensure_ascii=False,indent=1)
