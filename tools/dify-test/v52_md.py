...
d=json.load(open(f"docs/loop/v52/{s}/raw_{s}_1.json",encoding="utf-8"))
rows=d["turns"]
...
o+=["","## 各回の返事の先頭10文字","","| 通 | 先頭10文字 | 「さん」で始まる |","|---|---|---|"]
for r in rows:
    h=r["answer"][:10].replace("\n","⏎")
    o.append(f"| {r['turn']} | `{h or '（0字）'}` | {'**はい**' if r['answer'].startswith('さん') else 'いいえ'} |")
o+=["","## 全文",""]
for r in rows:
...
open(f"docs/loop/v52/{s}/{name}_出力.md","w",encoding="utf-8").write("\n".join(o)+"\n")
