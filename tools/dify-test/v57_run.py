...
for turn in range(1,MAXT+2):
    ...
    if turn==6 and name=="V":
        stop="6通目の返事を受け取りました。指示書のとおり、ここで止めます"
...
os.makedirs(f"docs/loop/v57/{name}",exist_ok=True)
json.dump({"script":name,"user":USER,"conversation_id":conv,"closed_at":closed_at,"turns":rows},
          open(f"docs/loop/v57/{name}/raw_{name}_1.json","w",encoding="utf-8"),ensure_ascii=False,indent=1)
