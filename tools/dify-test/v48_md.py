...
d=json.load(open(f"docs/loop/v48/{s}/raw_{s}_1.json",encoding="utf-8"))
rows=d["turns"]
...
for r in rows:
    o.append("| {} | {} | {} | {} | {} | {} | {} | {} | `{}` | `{}` | `{}` | `{}` | `{}` | `{}` | `{}` | `{}` | `{}` | `{}` | `{}` | {} | {} | {} |".format(
        ...
        g(r,"期限切れの解除","locked"),g(r,"期限切れの解除","rescore"),g(r,"期限切れの解除","urgent"),
        g(r,"期限切れの解除","crisis_word"),
...
for r in rows:
    ...
    if s=="R":
        ...
        elif d.get("consent_turn") and r["turn"]==d["consent_turn"]+2: tag="（＋2通目＝危ないことば）"
    o+=[f"### {r['turn']}通目{tag}（{jt(r)} JST）",f"**送った文**：{r['query']}","","**返事：**","```",
        (a if a.strip() else "（返事が返ってきませんでした。0字です）"),"```",""]
open(f"docs/loop/v48/{s}/{name}_出力.md","w",encoding="utf-8").write("\n".join(o)+"\n")
