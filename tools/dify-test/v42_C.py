...
user="t-42-C"; conv=None; rows=[]; prev=""; si=0; stop=""
...
for turn in range(1,22):
    ...
    if turn==1:
        ...
        if not any("2026-09-20h" in t for t in mark):
            stop="版ちがい"; print("★ 版の目印が 2026-09-20h ではありません。止めます。",flush=True); break
...
os.makedirs("docs/loop/v42/C",exist_ok=True)
json.dump({"script":"C","user":user,"conversation_id":conv,"consent_turn":consent_turn,"turns":rows},
          open("docs/loop/v42/C/raw_C_1.json","w",encoding="utf-8"),ensure_ascii=False,indent=1)
print("会話ID:",conv)
