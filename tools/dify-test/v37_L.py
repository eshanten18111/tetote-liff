...
user="t-37-L"; conv=None; rows=[]; fi=0; prev=""; stop=""
for turn in range(1,27):
    ...
    sp=[o["outputs"] for o in r["outs"] if "支援必要度" in o["title"] and o["outputs"]]
    la=(ex[0].get("length_alerted") if ex else "?")
    cl=(ex[0].get("closing") if ex else "?")
    cr=(sp[0].get("closing_reason") if sp else "?")
    hs=[o["outputs"] for o in r["outs"] if "返信整形" in o["title"] and o["outputs"]]
    fc=(hs[0].get("forced_close") if hs else "?")
    i6=[o["outputs"] for o in r["outs"] if "IF/ELSE 6" in o["title"] and o["outputs"]]
    i6v=(i6[0].get("result") if i6 else "-")
    mark=[t for t in r["titles"] if "定数生成" in t]
    print(f"--- {turn}通目 送={q} / {r['elapsed']}秒 / {len(r['answer'])}字 / node{len(r['titles'])} / 版={mark} / closing={cl!r} / closing_reason={cr!r} / length_alerted={la!r} / forced_close={fc!r} / IF6={i6v} / need={(pa[0].get('support_need') if pa else '-')}",flush=True)
    ...
    if turn==1:
        ...
        if not any("2026-09-20c" in t for t in mark):
            stop="版ちがい"; print("★ 版の目印が 2026-09-20c ではありません。止めます。",flush=True); break
    if (cl is True or cl=="true") and turn<25:
        stop=f"closing が true（{turn}通目）"
        print(f"★ {turn}通目で closing が true になりました。25通目より前なので、ここで止めます。",flush=True); break
    time.sleep(1)
print("止めた理由:",stop or "26往復を流しきりました")
os.makedirs("docs/loop/v37/L",exist_ok=True)
json.dump({"script":"L","user":user,"conversation_id":conv,"turns":rows},
          open("docs/loop/v37/L/raw_L_1.json","w",encoding="utf-8"),ensure_ascii=False,indent=1)
print("会話ID:",conv)
