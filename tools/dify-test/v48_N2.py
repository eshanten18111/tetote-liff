# -*- coding: utf-8 -*-
"""第48版 台本N2：引き継ぎに至らない相談で、お知らせが出る位置を見る（31往復）。
...
def grab(r,sub,key,exact=False):
    ...
    return None
user="t-48-N2"; conv=None; rows=[]; fi=0; prev=""; stop=""
...
for turn in range(1,32):
    ...
    print(f"--- {turn}通目 送={q} / {hm(datetime.datetime.fromisoformat(r['sent_jst']))} / {r['elapsed']}秒 / {len(r['answer'])}字 / node{len(r['titles'])} / 版={mark} / IF6={i6} / IF7={i7} / IF18={i18} / locked={lk!r} / rescore={rs!r} / urgent={ur!r} / crisis_word={cw!r} / closing={cl!r} / reason={cr!r} / left={lf!r} / near={nr!r} / phase={ph!r} / turn_notice={tn!r} / forced_close={fc!r} / need={sn} / risk={sr} / HTTP={http}",flush=True)
...
os.makedirs("docs/loop/v48/N2",exist_ok=True)
json.dump({"script":"N2","user":user,"conversation_id":conv,"turns":rows},
          open("docs/loop/v48/N2/raw_N2_1.json","w",encoding="utf-8"),ensure_ascii=False,indent=1)
