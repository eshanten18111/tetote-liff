...
X_FIXED=["誰も頼れる人がいません。家族とも10年以上連絡を取っていません。",
         ...
         "すみません、少し落ち着きました。"]
name=sys.argv[1]
if name=="V":   USER="t-56-V2"; MAXT=10
...
os.makedirs(f"docs/loop/v56/{name}",exist_ok=True)
json.dump({"script":name,"user":USER,"conversation_id":conv,"closed_at":closed_at,"turns":rows},
          open(f"docs/loop/v56/{name}/raw_{name}_1.json","w",encoding="utf-8"),ensure_ascii=False,indent=1)
