# -*- coding: utf-8 -*-
"""第52版 台本ランナー。使い方: python3 v52_run.py Q2|S"""
...
name=sys.argv[1]
if name=="Q2":  user="t-52-Q2"; MAXT=10; ASK=set()
elif name=="S": user="t-52-S";  MAXT=25; ASK={20,22}
else: raise SystemExit("Q2 / S のどちらかを指定してください")
...
for turn in range(1,MAXT+2):
    ...
    if r["answer"].startswith("さん"):
        print(f"★ {turn}通目の返事が「さん」で始まっています（S-1 に関わります）。",flush=True)
    fc=g(r,"返信整形","forced_close")
    if fc and ("さん" in str(fc) or "締め" in str(fc)):
        print(f"[forced_close] {turn}通目: {fc!r}",flush=True)
    ...
    if cl is True or cl=="true":
        ...
        stop=f"{MAXT}通目まで送りましたが締まりませんでした。指示書のとおり、ここで止めます"
        print("☆ "+stop,flush=True); break
    time.sleep(1)
print("止めた理由:",stop or "流しきりました")
print("締まった回:",closed_at)
os.makedirs(f"docs/loop/v52/{name}",exist_ok=True)
json.dump({"script":name,"user":user,"conversation_id":conv,"closed_at":closed_at,"turns":rows},
          open(f"docs/loop/v52/{name}/raw_{name}_1.json","w",encoding="utf-8"),ensure_ascii=False,indent=1)
