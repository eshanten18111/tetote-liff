...
user="t-38-L"; conv=None; rows=[]; fi=0; prev=""; stop=""
for turn in range(1,27):
    ...
    if turn==1:
        ...
        if not any("2026-09-20d" in t for t in mark):
            stop="版ちがい"; print("★ 版の目印が 2026-09-20d ではありません。止めます。",flush=True); break
...
os.makedirs("docs/loop/v38/L",exist_ok=True)
json.dump({"script":"L","user":user,"conversation_id":conv,"turns":rows},
          open("docs/loop/v38/L/raw_L_1.json","w",encoding="utf-8"),ensure_ascii=False,indent=1)
print("会話ID:",conv)
