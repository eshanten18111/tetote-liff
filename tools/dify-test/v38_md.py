...
KEY=("次にお話しできるのは","区切らせて","一度お休み","今日はここまで","次にお話しできる時",
     "次にまたお話し","今日はたくさん","今日はここで","お話しくださって、ありがとう",
     "今日はここまでに","またお話しできる","本日はここ","今日はこのあたり")
BAN=("いつでもお声がけください","いつでもご連絡ください","いつでもお待ちしています")
rows=[]
for f in sorted(glob.glob("docs/loop/v38/L/raw_L_*.json")):
...
out.append(f"- 20〜24通目で電話2件が出た回（L-10）： {[r['turn'] for r in rows if 20<=r['turn']<=24 and ('いのちSOS' in r['answer'] or 'よりそい' in r['answer'])] or 'なし'}")
...
open("docs/loop/v38/L/台本L_出力.md","w",encoding="utf-8").write("\n".join(out)+"\n")
print("\n".join(out[:60]))
