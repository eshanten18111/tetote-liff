...
def send(q, record=False):
    ...
    t0=time.time(); t_last=t_end=t_ans=None; after=[]
    ...
    for line in p.stdout:
        ...
        if e=="message": t_last=now
        elif e=="message_end": t_end=now
        elif e=="node_finished":
            ttl=d.get("title")
            if ttl=="回答": t_ans=now
            if t_last is not None and now>t_last:
                after.append((ttl, d.get("elapsed_time"), d.get("status")))
    p.wait()
    return {"t0":t0,"t_last":t_last,"t_end":t_end,"t_ans":t_ans,"after":after}
...
for i,q in enumerate(MEAS,1):
    r=send(q,True); r["turn"]=5+i; rows.append(r)
    print(f"  {5+i}通目 t_last-t0={r['t_last']-r['t0']:.1f} t_end-t0={r['t_end']-r['t0']:.1f} 差={r['t_end']-r['t_last']:.1f}",flush=True)

print()
print("通目 | t_last-t0 | t_end-t0 | 差(t_end-t_last) | 回答ノード終了-t0")
print("-----|-----------|----------|------------------|------------------")
for r in rows:
    ans=f"{r['t_ans']-r['t0']:.1f}" if r['t_ans'] else "-"
    print(f"{r['turn']:<4} | {r['t_last']-r['t0']:9.1f} | {r['t_end']-r['t0']:8.1f} | {r['t_end']-r['t_last']:16.1f} | {ans:>17}")
d=[r['t_end']-r['t_last'] for r in rows]; tl=[r['t_last']-r['t0'] for r in rows]; te=[r['t_end']-r['t0'] for r in rows]
...
print("=== 差の中で動いていたノード ===")
for r in rows:
    print(f"{r['turn']}通目 の差 {r['t_end']-r['t_last']:.1f}秒 の中身")
    if not r["after"]: print("   （なし）")
    for ttl,el,st in r["after"]: print(f"   {ttl}  {el}  status={st}")
print()
print("会話ID:",conv)
json.dump([{k:v for k,v in r.items()} for r in rows], open("v11_rows.json","w"), ensure_ascii=False, indent=1)
